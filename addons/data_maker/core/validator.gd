...
func has_warning(file: Dictionary, prop: String) -> bool:
	...
	if field_type in ["bool", "enum", "export_enum", "collection"]:
		return false
	var v = file["data"].get(prop)
	# Field absent from .tres but has a non-empty GDScript default → not a warning
	if v == null:
		var hint = gd_parser.get_hint(file, prop, store)
		var default_val = hint.get("default")
		if default_val != null and not (typeof(default_val) == TYPE_STRING and (default_val as String) == ""):
			return false
